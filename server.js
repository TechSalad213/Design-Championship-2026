...
app.post("/login", async (req, res) => {
    ...
    const user = database.users.find(
       ...
       // pass => user.passwordHash === password // 
    );
...
app.post("/uploadSkill", upload.any(), async (req, res) => {
    ...
    const { userId, skillName, learning, description, category, learningTime, requirements, topic } = req.body;
    ...
    const courseFile = req.files.find(file => file.fieldname === "courseFile");
    const stageFiles = req.files.filter(file => file.fieldname === "stageFile");

    stages.forEach((stage, i) => {
        stage.attachment = stageFiles[i] ? `/uploads/${stageFiles[i].filename}` : null;
    });
    ...
    database.skills[userId][skillId] = {
        ...
        topic: topic,
        courseFile: courseFile ? `/uploads/${courseFile.filename}` : null,
    };
